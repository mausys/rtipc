...
struct ri_rtipc
{
  ...
  ri_consumer_q_t **consumers;
  ri_producer_q_t **producers;
};
...
static ssize_t init_producers(ri_rtipc_t *rtipc, uintptr_t addr, uintptr_t addr_end, unsigned index_offset, bool shm_init)
{
  for (unsigned i = 0; i < rtipc->num_producers; i++) {
    ...
    rtipc->producers[i] = ri_producer_q_new(rtipc->shm, param, addr, shm_init);
...
static ssize_t init_consumers(ri_rtipc_t *rtipc, uintptr_t addr, uintptr_t addr_end, unsigned index_offset, bool shm_init)
{
  for (unsigned i = 0; i < rtipc->num_consumers; i++) {
    ...
    rtipc->consumers[i] = ri_consumer_q_new(rtipc->shm, param, addr, shm_init);
...
ri_rtipc_t* ri_rtipc_new(ri_shm_t *shm, bool shm_init)
{
  ...
  if (rtipc->num_consumers > 0) {
    rtipc->consumers = calloc(rtipc->num_consumers, sizeof(ri_consumer_q_t*));
  ...
  if (rtipc->num_producers > 0) {
    rtipc->producers = calloc(rtipc->num_producers, sizeof(ri_producer_q_t*));
...
fail_channels:
  for (unsigned i = 0; i < rtipc->num_producers; i++) {
    if (rtipc->producers[i])
      ri_producer_q_delete(rtipc->producers[i]);
  }

  for (unsigned i = 0; i < rtipc->num_consumers; i++) {
    if (rtipc->consumers[i])
      ri_consumer_q_delete(rtipc->consumers[i]);
  }
...
ri_consumer_q_t* ri_rtipc_take_consumer(const ri_rtipc_t *rtipc, unsigned idx)
{
  if (idx >= rtipc->num_consumers)
    return NULL;

  ri_consumer_q_t* consumer = rtipc->consumers[idx];
  rtipc->consumers[idx] = NULL;

  return consumer;
}

ri_producer_q_t* ri_rtipc_take_producer(const ri_rtipc_t *rtipc, unsigned idx)
{
  if (idx >= rtipc->num_producers)
    return NULL;

  ri_producer_q_t* producer = rtipc->producers[idx];
