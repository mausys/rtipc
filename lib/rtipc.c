...
ri_rtipc_t* ri_rtipc_new(ri_shm_t *shm, bool shm_init)
{
  ...
  if (rtipc->num_consumers > 0) {
    rtipc->consumers = calloc(rtipc->num_consumers, sizeof(ri_consumer_queue_t*));
  ...
  if (rtipc->num_producers > 0) {
    rtipc->producers = calloc(rtipc->num_producers, sizeof(ri_producer_queue_t*));
...
fail_channels:
  for (unsigned i = 0; i < rtipc->num_producers; i++) {
    if (rtipc->producers[i])
      ri_producer_queue_delete(rtipc->producers[i]);
  }

  for (unsigned i = 0; i < rtipc->num_consumers; i++) {
    if (rtipc->consumers[i])
      ri_consumer_queue_delete(rtipc->consumers[i]);
  }
...
ri_consumer_queue_t* ri_rtipc_take_consumer(const ri_rtipc_t *rtipc, unsigned idx)
{
  if (idx >= rtipc->num_consumers)
    return NULL;

  ri_consumer_queue_t* consumer = rtipc->consumers[idx];
  rtipc->consumers[idx] = NULL;

  return consumer;
}

ri_producer_queue_t* ri_rtipc_take_producer(const ri_rtipc_t *rtipc, unsigned idx)
{
  if (idx >= rtipc->num_producers)
    return NULL;

  ri_producer_queue_t* producer = rtipc->producers[idx];
